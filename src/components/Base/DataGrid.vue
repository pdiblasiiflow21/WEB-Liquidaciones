<template>
    <div class="data-grid">
        ...
        <basic-table :rows="rows" :showActions="showActions" :showCheck="showCheck" :checkFunction="checkFunction" v-if="hasColumns">            
            <slot></slot>
        ...
        <loading ref="loading" />
        <error-message   ref="errorM"></error-message>
    </div>
</template>
...
<script>    
    export default {
        ...
        methods: {
            ...
            getPage() {
                ...
                this.api[this.method](request).then(response => {
                    ...
                    this.$emit('onDataLoaded', this.rowCount);
                          ...
                          this.$refs.errorM.showError(err);
                    this.rows = [];
                    this.$refs.loading.hide();
            ...
            getFirstPage() {
                this.$refs.loading.show();
                let request = { ...this.filter, currentPage: 1, pageSize: this.rowsPerPage };
                if (typeof this.onBeforeLoad == 'function') { 
                         this.onBeforeLoad();
                    }       
                this.api[this.method](request).then(response => {
