...
<div class="clientes-container" v-if="canSeePage()">
    <div class="grid">
        <list-view ref="listView" resource="clientes" :onBeforeSave="onBeforeSave" :customValidations="customValidations" :showActions="true" :showEdit="true" :modalSize="modalSize" :showDelete="false" :searchBy="sada" :onBeforeEdit="onBeforeEdit" :filter=filtroGrillaOptions :disableSave="false">
            <template slot='title'>
                Clientes
            </template>
            ...
            <template slot="filters">
                <b-colxx :xxs="3">
                    ...
                    <div class="filter-sm">
                        <b-form-select style="border-radius: 25px;" placeholder="Estados de Facturación" v-model="filtroGrillaOptions.estadoFacturacion">
                            <option value="">Estados Facturación (Todos)</option>
                            <option v-for="(selectOption, indexOpt) in estadosFacturacion" :key="indexOpt" :value="selectOption.value">
                                {{ selectOption.text }}
                            </option>
                        </b-form-select>
                    </div>
                ...
                <b-row>
                    ...
                    <b-col>
                        <b-form-group label="Seleccionar estados para facturar">
                            <b-form-checkbox-group v-model="modalMetodoEnvio.estadosFacturacion" :options="estadosFacturacion" class="mb-3" value-field="value" text-field="text" disabled-field="notEnabled"></b-form-checkbox-group>
                        </b-form-group>
                    </b-col>
                    <b-col align-self="center">
...
export default {
    data() {
        return {
            ...
            metodosEnvio: [{
                ...
                {
                    value: 4,
                    text: 'Logística Inversa'
                },
                {
                    value: 6,
                    text: 'Puerta a Puerta Ju'
                },
                {
                    value: 8,
                    text: 'Fulfillment Refrigerado'
                }
            ],
            filtroGrillaOptions: {
                estadoFacturacion: ''
    ...
    methods: {
        ...
        setProvinceFilter(entity) {
            this.provinceFilter = {
                provinciaId: entity.provincia ? entity.provincia.id : 0
            };
        },
        setTipoDocumentoFilter(entity) {
            this.tipoDocumentoFilter = {
                tipoDocumentoId: entity.tipoDocumento ? entity.tipoDocumento.id : 0
            };
        },
        setTipoImpuestoFilter(entity) {
            this.provinceFilter = {
                tipoImpuestoId: entity.tipoImpuesto ? entity.tipoImpuesto.id : 0
            };
        },
        resetData() {
            this.resetFilters();
        ...
        agregarMetodoDeEnvio(item) {
            this.metodosDeEnvioAux.push({
                metodoEnvio: item.metodoEnvio,
                estadosFacturacion: item.estadosFacturacion
            });
        ...
        agregarItem(i) {
            this.impuestos.push({
                impuesto: {
                    descripcion: i.impuestos.descripcion,
                    id: i.impuestos.id,
        ...
        editItem(impuesto) {
            ...
            this.modalEditImpuesto = {
                index: editIndex,
                porcentajeExencion: this.maskForCulNum(impuesto.porcentajeExencion),
